...
|      continue-number      |             whether to init question number to one in every big question             |                       false                        |
...
If you want to hide a particular section from the score table, use `show-score-table: false` in the corresponding question type call (see Choice Question / Fill Question sections).
...
- `show-score-table` – whether to include this section’s score box in the master score table (default `true`).
- `show-fill-blank` – if `true`, draws a blank line after the question for manual answering (default `false`).
...
- `show-score-table` – control whether this section’s score box appears in the master score table.
